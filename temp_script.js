    ...
    async function saveData() {
        document.querySelectorAll('.group-title-input').forEach(input => input.setAttribute('value', input.value));
        ...
        if (selectedClass) {
            ...
            groupData[selectedClass] = document.getElementById('groups-container').innerHTML;
        }
        ...
        try {
            ...
            await db.ref("app_data/main_data").update(dataToSave);
    ...
    async function loadFromLocalStorage() {
        try {
            ...
            db.ref("app_data/main_data").on('value', (snapshot) => {
                if (snapshot.exists()) {
    ...
    function listenToStudentForms() {
        ...
        db.ref('student_forms').on('value', (snapshot) => {
            if(!snapshot.exists()) {
            ...
            formsArray.forEach(data => {
                    ...
                    for(const [key, val] of Object.entries(data.formResponse)) {
                        if(key === "Eksik Kazanım (Ödev)") {
                            answersHTML += `<div style="margin-top: 8px; background: rgba(239, 68, 68, 0.1); padding: 10px; border-radius: 6px; border-left: 4px solid var(--red); font-size: 13px; color: var(--text-main);">
                                <strong style="color: var(--red);"><i class="fas fa-exclamation-circle"></i> Öğrencinin Kişisel Ev Ödevi (Tik Atılamayanlar):</strong> <br> ${val}
                            </div>`;
                        } else {
                            answersHTML += `<div style="margin-top: 8px; background: var(--bg); padding: 8px; border-radius: 4px; font-size: 13px;">
                                <strong>Soru:</strong> ${key} <br> <strong>Cevap:</strong> ${val}
                            </div>`;
                        }
                    }
                }
                ...
                const dateStr = data.timestamp ? new Date(data.timestamp).toLocaleString('tr-TR') : (data.tarih || '');
                ...
                div.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom: 10px;">
                        <div>
                            <h4 style="margin:0 0 5px 0;">${data.adSoyad} <span style="font-size:12px; color:var(--text-muted);">(${data.sinif} - No: ${data.ogrenciNo})</span></h4>
    ...
    function getKonuOzeti(konu, dersAdi, unite) {
        ...
        if (d.includes('matematik') || d.includes('geometri')) {
            ...
            beceriler = ['Computational thinking', 'Problem ayrıştırma', 'Yaratıcı çözüm üretme'];
            ...
            beceriler = ['Eleştirel okuma ve yazma', 'Sözel iletişim', 'Yaratıcı üretim'];
...
`;
            ...
            if (useBloom) {
                promptText += `4. Bloom Taksonomisi (Dersin hedeflerini Bilme düzeyinden başlayarak Sentez/Oluşturma ve Değerlendirme düzeylerine kadar taşı, öğrencileri yaratıcı üretime teşvik et).\n`;
            }
            if (useAnaliz) {
...
- Okunaklı başlıklar, listeler ve tablolar kullanarak sun.`;
    ...
    function generateGroups() {
        ...
        if (typeof groupData === 'object') groupData[selectedClass] = '';
        
        groups.forEach((group, index) => {
            let html = `<div class="group-card"><h3><input type="text" class="group-title-input" value="Küme ${index + 1}" oninput="saveData()"><button class="action-btn btn-delete" onclick="if(confirm('Kümeyi silmek istiyor musunuz?')){this.closest('.group-card').remove(); updateStudentPickers(); saveData();}" title="Küme Sil"><i class="fas fa-times"></i></button></h3><ul style="padding:0;list-style:none;">`;
            group.forEach(m => {
    ...
    function addManualGroup() {
        ...
        groupCard.innerHTML = `<h3><input type="text" class="group-title-input" value="Küme ${groupIndex}" oninput="saveData()"><button class="action-btn btn-delete" onclick="if(confirm('Kümeyi silmek istiyor musunuz?')){this.closest('.group-card').remove(); updateStudentPickers(); saveData();}" title="Küme Sil"><i class="fas fa-times"></i></button></h3><ul style="padding:0;list-style:none; min-height:30px;"></ul>
            <div class="group-footer"><button class="btn btn-purple" style="font-size:11px; padding:8px;" onclick="addXPToGroup(this)"><i class="fas fa-users-cog"></i> Kümeye Toplu Puan Ver</button><div style="display:flex; gap:8px;"><select class="student-picker" style="flex:1; padding:8px; border-radius:8px; font-size:12px; border:1px solid var(--border); background:var(--input-bg); color:var(--text-main);"><option value="">Öğrenci Seç...</option></select><button class="btn" onclick="addStudentToGroupUI(this)" style="padding:8px 15px; font-size:12px;">Ekle</button></div></div>`;
    ...
    function clearAllGroups() { 
        if (confirm('Tüm grupları silmek istediğinize emin misiniz?')) { 
            ...
            if (selectedClass && typeof groupData === 'object') groupData[selectedClass] = '';
            document.getElementById('groups-container').innerHTML = ''; 
            saveData(); 
        } 
    }
    ...
    function loadClassGroups(className) {
        ...
        if (container) {
            container.innerHTML = (typeof groupData === 'object' ? groupData[className] : '') || '';
    ...
    function addXPToGroup(btn) {
        const amtStr = prompt("Kümeye eklenecek/çıkarılacak puan?", "10"); const amount = parseInt(amtStr); if (isNaN(amount)) return;
        const names = Array.from(btn.closest('.group-card').querySelectorAll('li .student-name-text')).map(s => s.getAttribute('data-name'));
        names.forEach(name => { 
            const idx = studentData.findIndex(s => s.adSoyad === name); 
            if (idx !== -1) { 
                if (!studentData[idx].xp) studentData[idx].xp = 0; 
                studentData[idx].xp += amount; 
                if (!studentData[idx].xpLogs) studentData[idx].xpLogs = [];
                studentData[idx].xpLogs.push({ reason: 'Küme / Toplu Etkinlik Puanı', amount: amount, date: new Date().toISOString() });
            } 
        });
        renderStudentTable();
        document.querySelectorAll('.group-card li').forEach(li => {
            const nameSpan = li.querySelector('.student-name-text'); if(nameSpan) { const student = studentData.find(s => s.adSoyad === nameSpan.getAttribute('data-name')); if (student) li.querySelector('.xp-badge').textContent = (student.xp || 0) + ' XP'; }
        });
        saveData(); alert(`Kümeye ${amount} XP dağıtıldı.`);
    }
    ...
    function renderSchedule() {
        ...
        times.forEach(time => {
            ...
            days.forEach(day => {
                const entries = scheduleData.filter(s => s.day === day && s.time === time); let content = '';
                entries.forEach(e => {
                    content += `<div style="background:var(--secondary); color:white; padding:5px 8px; border-radius:8px; margin-bottom:5px; font-size:12px; display:flex; justify-content:space-between; align-items:center;">
                        <span>${e.lesson}</span><button onclick="removeFromSchedule('${day}', '${time}', '${e.lesson}')" style="background:none; border:none; color:white; cursor:pointer;"><i class="fas fa-times"></i></button></div>`;
                });
    ...
    function removeFromSchedule(day, time, lesson) { if (confirm('Dersi silmek istediğinize emin misiniz?')) { scheduleData = scheduleData.filter(s => !(s.day === day && s.time === time && s.lesson === lesson)); renderSchedule(); saveData(); } }
    function loadScheduleExcel() {
        ...
        const reader = new FileReader(); reader.onload = function(e) {
            ...
            if (confirm("Mevcut program silinerek aktarılacak?")) {
                scheduleData = [];
                rows.forEach(row => {
                    const day = row.Gün || row.Gun || ""; const time = row.Saat || ""; const lesson = row.Ders || row.Sınıf || row.Sinif || "";
    ...
    function addTrackingItem() {
        ...
        const selectedClasses = Array.from(document.querySelectorAll('.track-class-checkbox:checked')).map(cb => cb.value);
    ...
    function renderTrackingList() {
        ...
        trackingData.forEach(item => {
            ...
            card.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <button class="btn" style="background:#25D366; padding:5px 12px; font-size:11px;" onclick="shareTrackingSummary('${item.id}')"><i class="fab fa-whatsapp"></i> Özet</button>
    ...
    function openEvaluation(id) {
        ...
        students.forEach(s => {
            ...
            tr.innerHTML = `<td>${s.no} - ${s.adSoyad}</td>
                ...
                <td><button class="action-btn" onclick="shareStudentResult(${id}, '${s.adSoyad}')" style="color:#25D366; background:none; font-size:16px;"><i class="fab fa-whatsapp"></i></button></td>`;
            body.appendChild(tr);
        });
    ...
    function ileriGrupAnalizEt() {
        ...
        const skorlar = hedefOgrenciler.map(s => {
            ...
            // XP skoru (0-100 normalleştirilmiş)
            if (kritXP) {
                const maxXP = Math.max(...hedefOgrenciler.map(x => x.xp || 0), 1);
                const xpNorm = ((s.xp || 0) / maxXP) * 100;
        ...
        let resultHtml = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;flex-wrap:wrap;gap:8px;">
                ...
                <div style="display:flex;gap:8px;">
                    ...
                    <button class="btn" style="font-size:11px;padding:5px 12px;background:#25D366;" onclick="ileriGrupWhatsapp()"><i class="fab fa-whatsapp me-1"></i>Velilere Gönder</button>
                </div>
            </div>
        ...
        gruplar.forEach((grup, gi) => {
            ...
            grup.forEach((ogr, oi) => {
                ...
                const skorBar = Math.round(ogr._skor);
                resultHtml += `
...
_Proje Tabanlı Ders İşleme Sistemi — otomatik oluşturulmuştur._`;
    ...
    function pgListenToForms() {
        db.ref('project_group_forms').on('value', (snapshot) => {
            if(!snapshot.exists()) {
                pgFormsData = [];
    ...
    function pgSave() {
        try {
            ...
            db.ref("app_data/main_data").update({
  ...
  { "name": "Öğrenci Adı", "score": 85, "aktif": true }
    ...
    function pgRenderMessages() {
        ...
        gForms.forEach(f => {
            ...
            <strong>Gönderen:</strong> ${f.adSoyad}<br>
            <strong>Çözüm Fikri:</strong> ${f.formResponse.cozumOneri}<br>
            <strong>Katkısı:</strong> ${f.formResponse.kisiselKatki}`;
